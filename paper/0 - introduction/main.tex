\chapter{Introduction: Plasmas and Tokamak Plasmas}
    ...
    \begin{remark}
        ...
        This thesis shall make use only of the definitions and identities that follow.
    \end{remark}

    \shortline

    \begin{enumerate}
        \item  A mass-dominant positive deuterium ion and negative electric phase only, indexed via $\pm$ (or $*_{\pm}$), in (approximate) balance in particle density to give quasineutrality.

        \item  The dimensionless constants, with approximate values calculated according the JET operational values given in \cite{Wes00}:
        \begin{itemize}
            \item  Cyclotron numbers, $\rmCy_{\pm}$:
            \begin{align}
                \rmCy\!_{+}  \approx  5.327\ldots\times 10^{2},  &&
            ...
            \item  Knudsen numbers, $\rmKn_{\pm_{1}\pm_{2}}$, satisfying the dominant balance relations:
            \begin{equation}
                \max\{\rmKn_{\pm+},  \rmKn_{\pm-}\}  =  |\rmCy_{\pm}|
            \end{equation}
        ...
        \begin{itemize}
            ...
            \item  (Leading-order) plasma mass and momentum density, $\rho_{\rmM}(\bfx; t)  \in  \bbR$ and $\bfp(\bfx; t)  \in  \bbR^{3}$ respectively, defined:
            \begin{align}
                \rho_{\rmM}  :=  \int_{\bfv}f_{+},  &&
                       \bfp  :=  \int_{\bfv}f_{+}\bfv
            \end{align}

            \item  (Leading-order) plasma pressure, $p(\bfx; t)  \in  \bbR$, defined:
            \begin{equation}
            ...
            \begin{align}
                \rho_{\rmC}  :=  \frac{\beta\rmCy_{+}}{2\rmM^{2}}\int_{\bfv}[f_{+} - f_{-}],  &&
                       \bfj  :=  \frac{\beta\rmCy_{+}}{2}\int_{\bfv}[f_{+} - f_{-}]\bfv
            \end{align}
        ...
        \begin{itemize}
            ...
            \begin{multline}
                ...
                =  - \int_{\bfv}\left[\frac{1}{{\rmRef}_{++}}\bfdelta\bfC_{++} + \cdots + \frac{1}{{\rmRef}_{--}}\bfdelta\bfC_{--}\right]\cdot(\bfv - \bfu)  \label{eqn:energy conservation introduction}
            \end{multline}
