\section{Modeling: Theory and Simulation}
    ...
    When the same steps are applied for a quasineutral plasma, one derives the magnetohydrodynamic (MHD) equations. \BA{(Small MHD literature review- ask Patrick and Wayne.)}
    ...
    For practical purposes therefore, one requires some form of trade-off between a fully fluid and fully kinetic model. One common such technique is the gyrokinetic model \cite{Howes_et_al_2006, Parra_Barnes_Peters_2011, Abel_et_al_2013} which has been seen to be effective in modelling many kinds of plasma turbulence \cite{McKee_et_al_2001} but can exhibit non-physical behavior over long time periods \BA{[Ref]} among other physical weaknesses \BA{[Ref]}. \BA{Other examples of such models? Ask Wayne.}
    
    In this thesis, we shall consider a coupled $\delta f$ low-noise correction method. (See Chapter \ref{cha:delta f correction}) To derive this model, we recap the derivation of the classical MHD model, and overivew where it fails for highly kinetic tokamak plasmas.
