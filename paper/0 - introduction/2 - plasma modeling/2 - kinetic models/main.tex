\subsection*{Kinetic Models}
    ...
    \end{equation}
    where the collision operators $(\calC_{s}(\bfx, \bfv; t))_{s}$ are terms characterising the Coulomb collisions, defined as
    \begin{multline}\label{eqn:collision operator}
        \calC_{s}  :=  \frac{q_{s}}{m_{s}}\nabla_{\bfv}\cdot\left[\sum_{s'}q_{s'}\int_{\bfx' \neq \bfx, \bfv'; t'}\left(\widetilde{f_{s}}(\bfx, \bfv; t)\widetilde{f_{s'}}(\bfx', \bfv'; t') - \widetilde{f_{s}f_{s'}}[(\bfx, \bfv; t), (\bfx', \bfv'; t')]\right)  \right.  \\
        \left.{\color{white} \int}  [\bfk_{\bfE}(\bfx' - \bfx, \bfv'; t' - t) + \bfv\wedge\bfk_{\bfB}(\bfx' - \bfx, \bfv'; t' - t)]\right]
    \end{multline}
    where in turn:
    \begin{itemize}
