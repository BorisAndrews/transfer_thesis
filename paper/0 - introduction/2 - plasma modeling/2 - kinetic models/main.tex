\subsection{Kinetic Models}
    ...
    The evolution equations for $(f_{s})_{s}$ or $(\widetilde{f}_{s})_{s}$ are not so simple however, due to their non-linearity. When applying this technique we derive the ``\emph{Boltzmann equation}'', \BA{[Ref]}
    ...
    \begin{proof}
        ...
        \BA{Does this need some approriate decay condition on $(f_{s})_{s}$ so that I can apply integration by parts that I've forgotten to mention? In fact, the same is true later for the collision operators $(\bfC_{ss'})_{ss'}$ when I'm deriving the fluid model too, I should go through and check what decay conditions need mentioning.}
    \end{proof}
