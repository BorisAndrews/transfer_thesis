\chapter{Hilbert Complexes}\label{sec:Hilbert complexes}
    ...
    \section{(3+1)D in Space and Time}
        ...
        \subsection*{$H_{\bfgamma\bfomega}\Lambda^{\bullet}(\bfOmega\otimes[0, T])$}
            ...
            {\scriptsize \begin{center}\begin{tikzpicture}[align = center, node distance = 4cm, auto]
                ...
                \node (HL2) at (7.5,  0) {$\bfH_{\bfgamma\bfomega}\left(\begin{matrix} * - \rmdiv \\ \bfcurl + \partial_{t} \end{matrix}\right)$};
                \node (HL3) at (11.5, 0) {$\bfH_{\bfgamma\bfomega}(\partial_{t} + \rmdiv)$};
                \node (HL4) at (14,   0) {$L^{2}$};
