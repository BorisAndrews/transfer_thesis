\section{Numerical Models}\label{cha:FEM vs. PiC}
    ...
    \begin{itemize}
        ...
        This can be taken further when using finite elements in time (FET)---discussed further in Subsubsection \ref{cha:FET}---when testing (in $L^{2}$) against test functions $\varphi(\bfx; t)$ in test spaces defined over the whole space-\emph{time} domain, as
        \begin{multline}
        ...
        \begin{remark}[Testing against stochastic processes]
            Since $\bfX_{\pm}^{(i)}(t)$ and $\bfV_{\pm}^{(i)}(t)$ are complex stochastic processes, further care must be taken regarding the continuity of the test spaces to ensure these RHS particle terms are well-defined. This I will tackle in the future, when I have investigated the coupled hybrid FEM-PiC model in more detail.
        \end{remark}
