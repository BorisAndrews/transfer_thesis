\section{$\delta\!f$ Models}\label{cha:delta f models}
    ...
    This following result is crucial for deriving the fluid model from the Boltzmann equations, as the time derivative in $(\delta\!f_{s})_{s}$ must vanish after taking the relevant moments:

    \begin{theorem}[Conservation of moments on $(\delta\!f_{s})_{s}$]
    ...
    \begin{proof}
        \begin{enumerate}
            ...
            \begin{remark}
                \BA{I want to say this holds from the above statement, but I wouldn't be so naïve; I've done rudimentary uniqueness claims in ODEs of course. Some level of Lipschitz regularity needs to be assumed on the system for this to hold, the exact conditions of which I need to investigate further. Something for the further work section I think!}
            \end{remark}
        \end{enumerate}
    \end{proof}

    \BA{Why is this lemma important when talking about numerical models too? Primarily important as it means the definition is well-posed!}
