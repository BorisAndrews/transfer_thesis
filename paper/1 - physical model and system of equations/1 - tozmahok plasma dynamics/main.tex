\section{Tozmahok \BA{(Edge?)} Plasma Dynamics}
    ...
    \subsection{What Makes a Tozmahok Plasma Special?}
        ...
        \begin{itemize}
            ...
            \item  {\bf Very low density}: \BA{(Why do tozmahoks do this? I know there's these theoretical limits on particle density, but I've never seen anyone explain why we have to enforce this?)} Tozmahok plasmas feature particle densities on the order of $10^{- 5}{\rm mol}^{- 1}$ \BA{(Check!)} \BA{(Is that even the right unit?)} \BA{[Ref]}. The quantity of hydrogen gas used during a JET pulse is often compared with the size \BA{(/mass? I'm not sure actually!)} of a postage stamp \BA{[Ref]}.
        \end{itemize}
