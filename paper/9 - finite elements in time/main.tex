\chapter{Finite Elements in Time}\label{cha:finite elements in time}
    ...
    \begin{example}
        ...
        \begin{equation}\label{eqn:timestepper matrices}
            ...
            \end{array}\right)
        \end{equation}
        for $\bfA  \in  \bbR^{q\times (q - 1)}$, $\bfb  \in  \bbR^{q}$. (See Figure \ref{fig:timestepper matrices}) \BA{(Capitalization on ``Figure''?)}
        ...
        \begin{figure}[!ht]
            ...
            \end{subfigure}
            \caption{Sparsity plots for the two ``timestepping matrices'' when $q = 10$ on 3 (equal) intervals, under simple normalised polynomial bases. \\ Color indicates $\log(|*|)$ for the matrix entries. Black lines indicate block structure from (\ref{eqn:timestepper matrices}), while solid black lines indicate lower-triangular structure after elimination of first column with strong initial condition imposition.}
