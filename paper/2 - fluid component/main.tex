\chapter{Fluid Component}
    ...
    \end{align}
    and eliminating $\bfp  =  \rho_{\rmM}\bfu$, $p  =  \rho_{\rmM}\theta$ to give the final compressible MHD system in $\rho_{\rmM}$, $\bfu$, $\theta$, $\bfB$ only as shown in Figure \ref{fig:compressible strong form}. Gauss's law, $\nabla\cdot\bfB  =  0$ is enforced on the initial conditions (ICs) at $t  =  0$ only, and holds for later $t  >  0$ by Faraday's law, $\partial_{t}\bfB  =  - \nabla\wedge\bfE$. (See Section \ref{cha:structures})
    ...
    \input{2 - fluid component/1 - structures/main.tex}
