\chapter{Fluid Component}\label{cha:fluid component}
    ...
    \begin{remark}[Inclusion of bulk viscosity]
        In hindsight, I have no reason not to include bulk viscous effects, proportional to $\nabla[\rho_{\rmM}\nabla\cdot\bfu]$. They have very little effect on the analysis. It would be counterproductive for me to go through and re-introduce it throughout this section, however in future work I will elect not to omit it.
    \end{remark}
    ...
    \end{align}
    Due to the non-relativistic assumption, $\rho_{\rmC}$ is no longer present in these equations, i.e. the plasma is functionally \emph{``quasineutral''}.
    ...
    This system is completed by the thermal equilibrium identity (\ref{eqn:thermal equilibrium identity}), which can be interpreted here as an identity for $\bfj$. Assuming the LB collision operator model from Definition \ref{def:LB collision operators}, and taking again the non-relativistic assumption, $\rmM  \ll  1$, we derive the following identity:
    \begin{equation}
    ...
    \input{2 - fluid component/1 - structure preservation/main.tex}
    \input{2 - fluid component/2 - Navier--Stokes/main.tex}
    \input{2 - fluid component/3 - MHD structures/main.tex}
    \input{2 - fluid component/4 - MHD/main.tex}
    \input{2 - fluid component/5 - linearization and well-posedness/main.tex}
    \input{2 - fluid component/6 - numerics/main.tex}
