\chapter{Fluid Component}
    ...
    \end{equation}
    where the magnetic Reynolds number, $\rmRem$, and Hall number, $\rmRH$, are dimensionless constants also. \BA{(With definitions...)}
    ...
    \end{align}
    and eliminating $\bfp  =  \rho_{\rmM}\bfu$, $p  =  \rho_{\rmM}\theta$ to give the final compressible MHD system in $\rho_{\rmM}$, $\bfu$, $\theta$, $\bfB$ only as shown in Figure \ref{fig:compressible strong form}.
