\chapter{Numerical Implementation}
    ...
    \begin{itemize}
        ...
        \item[\ast\textcolor{darkyellow}{$^{\bf 2}$}]  There's various possibilities here. Patrick mentioned {\tt DMSwarm} in {\tt PETSc},\footnote{Will Saunders has concerns about the way {\tt DMSwarm} passed particles between MPI ranks- the particles are distributed from one rank to \emph{all possible receiving ranks}---computationally intense already!---then has no checks for existence and uniqueness for a receiving rank. Apparently solving this would be a parallelization nightmare too.} Pablo mentioned a few other ideas and people to get in contact with about this- I'm confident that some of the ideas I'd like to try should already be supported in some form, \emph{without} me having to delve into the nitty-gritty of parallelising the PIC.
