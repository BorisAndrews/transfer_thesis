\subsubsection*{Stochastic Gyrokinetic Theory}
    ...
    where similarly:
    \begin{itemize}
        \item  $\bfu$, $\lambda_{\pm}$, $\sigma_{\pm}$ are evaluated at the gyrocenter, $\bfx_{\rmg}$
        \item  $V_{\parallel}$, $V_{\perp}$ are defined analogously to $v_{\parallel}$, $v_{\perp}$
    \end{itemize}
    and $\widetilde{\bfW}$ is a distinct Wiener process, independent of $\bfW$. \BA{(Proof of independence?)}
